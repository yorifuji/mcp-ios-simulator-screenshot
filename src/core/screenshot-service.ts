...
import { IScreenshotService } from './interfaces/screenshot.js';
import { IOutputDirectoryManager } from './interfaces/output.js';
import { IDeviceValidator } from './interfaces/device.js';
...
export class ScreenshotService implements IScreenshotService {
   ...
   * @param deviceValidator Device validator
   */
  constructor(
    private outputManager: IOutputDirectoryManager,
    private deviceValidator: IDeviceValidator
  ) {}
  ...
  public async captureScreenshot(options: ScreenshotOptions = {}): Promise<ScreenshotResult> {
    try {
      ...
      if (options.deviceId && options.deviceId !== 'booted') {
        const validationResult = await this.deviceValidator.validateDeviceId(options.deviceId);
        if (!validationResult.isValid) {
          throw new Error(validationResult.errorMessage || `Invalid device ID: ${options.deviceId}. Please use a valid device ID or 'booted'.`);
        }
      }
